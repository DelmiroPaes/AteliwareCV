...
/**********************************************************************************************//**
 * \fn  bool ParseAndCheckCommandLine(int argc, char* argv[])
 *
 * \brief Parse and check command line
 *
 * \author  Delmiro Paes
 *
    ...
    while (true)
    {
      ...
      for (size_t i = 0; i < prev_detection_results.size(); i++)
      {
        ...
        if (!FLAGS_no_smooth)
        {
          ...
          if ((face == nullptr) || ((face != nullptr) && ((std::abs(intensity_mean - face->_intensity_mean) / face->_intensity_mean) > 0.07f)))
          {
      ...
      {
        out.str("");
        out << "Performance: " << std::fixed << std::setprecision(2) << 1000.f / (timer["total"].getSmoothedDuration()) << " fps";
        cv::putText(prev_frame, out.str(), cv::Point2f(10, 45), cv::FONT_HERSHEY_TRIPLEX, 1.2, cv::Scalar(255, 0, 0), 2);
